...
__global__ void
    ...
    kernel_batched_gemm_xdl_cshuffle_v3(typename GridwiseGemm::Argument karg)
{
#if(!defined(__HIP_DEVICE_COMPILE__) || defined(__gfx9__))
    __shared__ char p_shared[GridwiseGemm::GetSharedMemoryNumberOfByte()];

    auto compute_ptr_offset_of_batch = typename GridwiseGemm::ComputePtrOffsetOfStridedBatch(
        karg.BatchStrideA, karg.BatchStrideB, karg.BatchStrideC, karg.BatchStrideScaleB);
    ...
    const long_index_t b_scale_batch_offset=__builtin_amdgcn_readfirstlane(
        static_cast<long_index_t>(compute_ptr_offset_of_batch.GetSacleBPtrOffset(g_idx)));

    printf("block_id: %d, a_batch_offset: %ld, b_batch_offset: %ld, c_batch_offset: %ld, b_scale_batch_offset: %ld\n",
           get_block_1d_id(), a_batch_offset, b_batch_offset, c_batch_offset, b_scale_batch_offset);
...
struct GridwiseGemm_xdl_cshuffle_v3
{
    ...
    struct Problem
    {
        __host__ Problem(index_t M_,
            ...
            : M{M_},
              ...
              BatchStrideScaleB{BatchStrideScaleB_},
              Batch{Batch_},
        ...
        __host__ void Print() const
        {
            std::cout << "problem {"
                      ...
                      << "BStrideA:" << BatchStrideA << ", "
                      << "BStrideB:" << BatchStrideB << ", "
                      << "BStrideC:" << BatchStrideC << ", "
                      << "BStrideScaleB:" << BatchStrideScaleB << ", "
                      << "Batch:" << Batch << ", "
                      << "MP:" << MPadded << ", "
                      << "NP:" << NPadded << ", "
                      << "KRead:" << KRead << ", "
    ...
    struct Argument : public tensor_operation::device::BaseArgument, public Problem
    {
        __host__ Argument(const ADataType* p_a_grid_,
            ...
            : Problem{M_, N_, K_, StrideA_, StrideB_, StrideC_, StrideScaleB_, BatchStrideA_,
                      BatchStrideB_, BatchStrideC_, BatchStrideScaleB_, Batch_},
              p_a_grid{p_a_grid_},
              p_b_grid{p_b_grid_},
              p_c_grid{p_c_grid_},
    ...
    {
        __device__ ComputePtrOffsetOfStridedBatch(index_t BatchStrideA,
                                                  index_t BatchStrideB,
                                                  index_t BatchStrideC,
                                                  index_t BatchStrideScaleB)
